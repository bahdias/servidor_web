...
int main() {
    ...
    while (1) {
        ...
        if (child_pid < 0) {
            ...
            close(client_socket);
            continue;
        } else if (child_pid == 0) { // Código do processo filho
            close(server_socket); // O processo filho não precisa do socket do servidor
            ...
            // Fecha o socket do cliente no processo filho
            close(client_socket);
            exit(0); // O processo filho deve encerrar após processar a requisição
        } else { // Código do processo pai
            close(client_socket); // O processo pai não precisa do socket do cliente
        }
    }

    // Nunca será executado, pois o loop while é infinito
    // Deixe o código aqui apenas para completar o exemplo
    close(server_socket);
